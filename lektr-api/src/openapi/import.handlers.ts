    ...
    try {
      ...
      for (const h of highlights) {
        ...
        try {
          ...
          highlightsImported++;
          existingByHash.set(contentHash, { id: '', content: h.content, page: null, chapter: null }); // Prevent duplicates within same import
  ...
  if (contentType.includes("multipart/form-data")) {
    try {
      ...
      for (const parsedBook of parsedBooks) {
        ...
        const existingHighlights = await db.select({ content: highlightsTable.content, originalContent: highlightsTable.originalContent })
