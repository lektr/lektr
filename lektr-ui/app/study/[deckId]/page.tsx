...
export default function StudyPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getStudySession(deckId, { limit: 20 }),
  ...
  const reviewMutation = useMutation({
    mutationFn: async ({ card, rating }: { card: StudyItem; rating: Rating }) => {
      if (card.isVirtual && card.highlightId) {
        return submitVirtualReview(card.highlightId, {
          rating,
          deckId,
          front: card.front,
          back: card.back,
      ...
      <AuthGuard>
        <div className="container py-8 max-w-[800px] mx-auto min-h-screen flex flex-col items-center justify-center text-center animate-fade-in">
          ...
          <div className="flex gap-4">
            <Link href={`/decks/${deckId}`} className="btn btn-secondary">
              Back to Deck
            </Link>
