...
export function TagSelector({ highlightId, bookId, currentTags, onClose }: TagSelectorProps) {
  ...
  const createTagMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setNewTagName("");
      // Auto-add the new tag to the highlight
  ...
  const addTagMutation = useMutation({
    mutationFn: ({ tagId }: { tagId: string }) =>
      addTagToHighlight(tagId, highlightId),
    onSuccess: () => {
  ...
  const removeTagMutation = useMutation({
    mutationFn: ({ tagId }: { tagId: string }) =>
      removeTagFromHighlight(tagId, highlightId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["book", bookId] });
